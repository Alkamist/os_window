...
flush :: proc(renderer: ^Microui_Renderer) {
    ...
    gl.BufferData(gl.ARRAY_BUFFER, renderer.write_index * 4 * size_of(renderer.vertices[0]), &renderer.vertices[0], gl.DYNAMIC_DRAW)
    gl.BufferData(gl.ELEMENT_ARRAY_BUFFER, renderer.write_index * 6 * size_of(renderer.indices[0]), &renderer.indices[0], gl.DYNAMIC_DRAW)
    ...
    gl.DrawElements(gl.TRIANGLES, i32(renderer.write_index * 6), gl.UNSIGNED_SHORT, nil)
